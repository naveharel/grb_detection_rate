...
plot displays* — the same shared model_day/model_night the surface uses, not
some other model or a stale evaluation.
...
A_log=-2.0, a value inside the slider's real range: log10(R_ztf) came back
just below the -2.0 floor, yet R_ztf was returned as a real float before the
fix).
...
CONFIGS = {
    ...
    "ztf_hc_preset":       {"optical_survey": True, "i_det": 6, "f_live": 0.17,
                            "A_log": -4.68, "omega_exp_deg2": 47.0, "t_overhead_s": 15.0},
...
def _reeval_on_sidebar_model(params: dict, N: float, t_cad_s: float):
    """The marker point as the displayed (sidebar) model_day/model_night
    combination gives it."""
    state = sb._build_models(params)
    return sb._eval_point(
        N, t_cad_s, state["i_det"], state["model_day"], state["model_night"],
        state["f_live"], state["f_live_night"], state["f_night"], state["optical_on"],
...
def test_markers_are_samples_of_the_displayed_model(name, overrides):
    ...
    marker is evaluated through a divergent path, or when a sub-floor marker
    leaks through without being hidden."""
...
def test_marker_follows_the_sidebar_schedule():
    """The ZTF marker moves WITH the surface when the schedule-relevant
    sliders move — guards against re-hard-coding a schedule into the marker
    evaluation via a separate aux model."""
    p1 = {**BASE, "optical_survey": True, "f_live": 0.08}
    p2 = {**BASE, "optical_survey": True, "f_live": 0.6}
    R1 = sb.compute_all(p1)["R_ztf"]
    R2 = sb.compute_all(p2)["R_ztf"]
    assert R1 is not None and R2 is not None
    assert abs(R1 - R2) > 1e-6 * max(R1, R2), (
        "R_ztf did not respond to the sidebar f_live — marker evaluated on a "
