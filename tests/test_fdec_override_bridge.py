...
def test_optical_mode_model_also_overridden(F0):
    """Optical mode used to build a second (model_night) instance that also
    needed the override applied — now there's a single model, but confirm
    the override still applies correctly when optical_survey=True."""
    k = 10.0
    st = sb._build_models(_params(optical_survey=True, F_dec_override_Jy=k * F0))
    assert st["fdec_override_applied"] is True
    assert st["model"].derived.F_dec_Jy == pytest.approx(k * F0, rel=1e-12)
