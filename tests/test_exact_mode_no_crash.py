...
on "Exact rate mode" in the live app. (This branch already fixed the
Pyodide-only int64/intp np.bincount crash independently — see
grb_detect/detection_rate.py's own "np.bincount needs intp indices" comments
— this test guards against that class of bug recurring.)
...
# App boot defaults (web/template.html slider defaults). Both budget keys are
# sent, exactly as readParams does — the bridge picks the mode's own (f_eff in
# optical mode, f_live otherwise).
BASE: dict = {
    "i_det": 2,
    "A_log": -4.68,
    "f_eff": 0.48,
...
PRESETS = {
    "default":    {},
    "ztf_public": {"optical_survey": True, "i_det": 2, "f_eff": 0.40,
                   "N_v": 2, "dt_v_h": 2.0, "A_log": -4.68,
                   "omega_exp_deg2": 47.0, "t_overhead_s": 15.0},
    "ztf_hc":     {"optical_survey": True, "i_det": 2, "f_eff": 0.40,
                   "N_v": 6, "dt_v_h": 1.0, "A_log": -4.68,
                   "omega_exp_deg2": 47.0, "t_overhead_s": 15.0},
    "rubin":      {"optical_survey": True, "i_det": 2, "f_eff": 0.7,
                   "N_v": 2, "dt_v_h": 0.5, "A_log": -7.0,
