...
\paragraph{Deferred refinements.} Optimiser search over $(N_v,\Delta t_v)$; ramp-averaged dominant mode (bracketed by the two conventions; exact mode has the ramp); closed-form mixture medians; per-band epochs.
...
This section compares the night-schedule model of Sec.~\ref{sec:schedule} against both the pre-schedule (one-timescale) model and the ZTF observations, at the two real observing modes.  Numbers are from \texttt{analysis/ztf\_validation.py} (run of 2026-08-19, saved in \texttt{analysis/validation\_output.txt}); the observational targets and the distance-yardstick decision (comoving $D_C$ as the primary measure for a rate-calibrated Euclidean model) are documented in \texttt{analysis/model\_gap\_report.md}.
...
\midrule
rate $\times\,\varepsilon_{\rm cov}$ [yr$^{-1}$] & 4.7 & 14.5 & 2.0 & 5.0 & 13.3 & 2.5 \\
$q_{\rm med}$ & 1.17 & 1.02 & $\lesssim 1.5$ & 1.14 & 1.05 & $\lesssim 1.5$ \\
on-axis-consistent ($q<1.5$) & 78\% & 88\% & $\ge 60\%$ & 79\% & 89\% & $\ge 60\%$ \\
$D_{\rm med}$ [Gpc, $D_C$] & 1.00 & 1.90 & 3.67 & 1.82 & 2.92 & 3.88 \\
$D_{90}$ [Gpc] & 1.41 & 3.91 & --- & 2.67 & 4.25 & --- \\
\bottomrule
\end{tabular}
\end{center}
