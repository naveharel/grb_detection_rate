...
$f_{\rm live}$ & 0.1\textsuperscript{\dag} & (unchanged) & -- & Fraction of $t_{\rm cad}$ spent observing (superseded in optical mode by $f_{\rm eff}$, Sec.~\ref{sec:schedule}) \\
$f_{\rm eff}$ & 0.48\textsuperscript{\dag} & (introduced 2026-08) & -- & Usable fraction of the night window; $f_{\rm live}=f_{\rm eff}\,f_{\rm night}$ (Sec.~\ref{sec:schedule}) \\
...
\medskip\noindent\textbf{Night-schedule generalisation.} With the two-timescale night schedule of Sec.~\ref{sec:schedule} both branches above become special cases of the single budget equation~\eqref{eq:budget_unified}, parameterised by the within-night usable fraction $f_{\rm eff}$ ($f_{\rm live}\equiv f_{\rm eff}\,f_{\rm night}$). The $f_{\rm live,eff}$ bookkeeping, the two separate model instances it required, and the $f_{\rm night}\ge f_{\rm live}$ constraint (now the trivial $f_{\rm eff}\le1$) are all retired.
...
\noindent Historically the continuous branch used a separate model instance (\texttt{model\_night}) with a rescaled live fraction, and the $\log_{10}(f_{\rm night})$ offset was applied post-hoc in log space. Under the night schedule of Sec.~\ref{sec:schedule} a single model instance covers both branches and the $f_{\rm night}$ factor is internal to the rate evaluation; the three validity bands above are unchanged ($N_v$ and $\Delta t_v$ act only on the discrete band).
...
\end{cases}
\]
so that $\sum_k g_k=t_{\rm cad}$ exactly. Two integers characterise how $i$ required detections wrap around nights:
\[
\mu=\left\lfloor\frac{i-1}{N_v}\right\rfloor,\qquad
\rho=(i-1)\bmod N_v ,
\]
and the \emph{span} of $i$ consecutive visits starting from visit $k$, $S_k=v_{k+i-1}-v_k$ (periodic extension $v_{k+mN_v}=v_k+m\,t_{\rm cad}$), takes only two values:
\begin{equation}
\label{eq:spans}
S_k=\begin{cases}
...
Two regions keep their legacy treatment unchanged: sub-day cadences ($t_{\rm cad}<1$\,day, the continuous band --- nightly revisits at spacing $t_{\rm cad}$, i.e.\ the degenerate schedule) and non-optical mode (no night structure). $N_v$ and $\Delta t_v$ act only on the discrete band.

\subsection{Unified exposure-time budget and $f_{\rm eff}$}
\label{sec:sched_budget}

\paragraph{Motivation for replacing $f_{\rm live}$.} The wall-clock live fraction $f_{\rm live}$ conflates two unrelated things: the astronomical night fraction $f_{\rm night}=t_{\rm night}/86400$ (geometry) and how much of the night the program actually uses (weather, moon, engineering, time-share). Define instead
\[
f_{\rm eff}\in(0,1]:\quad\text{usable fraction of the night window},\qquad f_{\rm live}=f_{\rm eff}\,f_{\rm night}.
\]
A useful diagnostic follows: \emph{different observing modes of the same telescope should have similar $f_{\rm eff}$} (same weather, same time-share) --- a large mismatch signals a modelling error. The legacy calibration indeed carried one: ZTF public and high-cadence needed $f_{\rm live}=0.08$ vs $0.17$ ($\times2.1$ apart), an artefact of the public mode's hidden second visit per night (below).
...
(i)~Discrete band, $N_v=1$: Eq.~\eqref{eq:budget_unified} is the legacy $t_{\rm exp}=f_{\rm live}t_{\rm cad}/N_{\rm exp}-t_{\rm OH}$ with $f_{\rm live}=f_{\rm eff}f_{\rm night}$.
(ii)~Continuous band: nightly revisits at spacing $\Delta t=t_{\rm cad}<t_{\rm night}$ mean $N_v=t_{\rm night}/\Delta t$ visits per night with period one day, and Eq.~\eqref{eq:budget_unified} gives
$t_{\rm exp}=f_{\rm eff}f_{\rm night}\cdot86400/(N_{\rm exp}N_v)-t_{\rm OH}=f_{\rm eff}\,\Delta t/N_{\rm exp}-t_{\rm OH}$
--- exactly the legacy $(f_{\rm live}/f_{\rm night})\,t_{\rm cad}/N_{\rm exp}-t_{\rm OH}$.
Both legacy model instances (\texttt{model\_day}, \texttt{model\_night}) therefore collapse into one model carrying $f_{\rm eff}$, and the legacy physicality constraint $f_{\rm night}\ge f_{\rm live}$ becomes the trivial $f_{\rm eff}\le1$ (the dynamic $t_{\rm night}$-slider floor is retired).
...
\item \textbf{Saturation}: $P_i=1$ exactly for $T\ge T_{100}=S_A+\max(G,\Delta t_v)$ (in the standard regime $G\ge\Delta t_v$: $T_{100}=S_A+G$, and one checks $S_B+\Delta t_v=S_A+G$ identically). At $N_v=1$: $T_{100}=i\,t_{\rm cad}$.
...
\]
and $t_+$ inverted from the single-phase decline exactly as in Eq.~\eqref{eq:D_window}. Every visit inside the window is above the flux limit, so counting visits is counting detections. The schedule machinery therefore composes with the existing window settings per channel (Sec.~\ref{sec:sched_dominant}): \texttt{win\_from\_peak} keeps its meaning (window measured from the peak), and \texttt{win\_i\_minus\_one} selects the optimistic span convention below.

\subsection{Dominant-term (analytic) mode: mixture of rectangles}
\label{sec:sched_dominant}

Group the schedule phases by the gap the burst lands in. There are at most three \emph{channels} $c$, each a group of identical starting gaps:
\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lcccl}
...
\end{tabular}
\end{center}
with phase weights $w_c=m_c\,g_c/t_{\rm cad}$, $\sum_c w_c=1$. The analytic rate is the \textbf{mixture of the existing piecewise rectangles} evaluated at per-channel required durations:
\begin{equation}
\label{eq:R_mixture}
R_{\rm dom}=\sum_c w_c\,R_{\rm rect}\bigl(T_{{\rm req},c}\bigr),
\qquad
T_{{\rm req},c}=
\begin{cases}
S_c+g_c & \text{legacy convention (worst-case wait)}\\
S_c & \texttt{win\_i\_minus\_one}\text{ (zero wait)},
\end{cases}
\end{equation}
where $R_{\rm rect}(T)$ is the unmodified A1--A7 machinery with $q_i(T)$, $D_i(T)$ (and, under \texttt{win\_from\_peak}, $D_w(q;T)$) substituted for their $i\,t_{\rm cad}$ forms. No new integrals appear; the model stays closed-form and piecewise.

\paragraph{Reduction and bracketing.} At $N_v=1$ there is a single channel with $w=1$ and $T_{\rm req}=i\,t_{\rm cad}$ (legacy) or $(i-1)\,t_{\rm cad}$ --- bit-identical to the legacy model, both conventions. In general, within channel $c$ the wait to the first visit is uniform on $[0,g_c]$, so
\[
\sum_c w_c\,\mathbf 1\{T\ge S_c+g_c\}\;\le\;P_i(T)\;\le\;\sum_c w_c\,\mathbf 1\{T\ge S_c\}
\]
pointwise: the two conventions of Eq.~\eqref{eq:R_mixture} are exact conservative/optimistic brackets of the phase ramp, generalising the legacy window-toggle bracket (Sec.~\ref{sec:window_settings}) channel-by-channel. The exact mode (Sec.~\ref{sec:sched_exact}) integrates the ramp itself.

\paragraph{Worked example --- ZTF public, continued.} With $\rho=1$, $N_v=2$ the $A_\Delta$ channel is empty and both surviving channels have the \emph{same} conservative duration, $S_A+G=S_B+\Delta t_v=t_{\rm cad}$: the conservative mixture is $R_{\rm rect}(t_{\rm cad}{=}2\,\mathrm d)$ --- already a factor-2 shorter requirement than the legacy $i\,t_{\rm cad}=4$\,d, because only \emph{one} inter-night wait is needed, not two. The optimistic mixture $w_{A_G}R_{\rm rect}(\Delta t_v)+w_B\,R_{\rm rect}(2\,\mathrm d-\Delta t_v)$ is dominated by the intra-night pair. The exact ramp lies between.

\paragraph{Regime classification and medians.} The displayed regime is that of the channel with the largest contribution $w_c\,R_c$ (at $N_v=1$: unchanged). The detected population is a mixture of the per-channel rectangle populations, so the closed-form medians of Eq.~\eqref{eq:filter_medians} no longer apply for $N_v\ge2$; \texttt{compute\_medians} takes the numerical path (the same forcing already used under \texttt{win\_from\_peak}).
...
\end{equation}
With the cuts off ($\tau\to+\infty$) Eq.~\eqref{eq:W_schedule} integrates $P_i$ of Eq.~\eqref{eq:P_i} exactly. Under \texttt{win\_from\_peak} the $T-S_c$ term implements the detection ramp; with it off, the detection window instead keeps the per-channel hard rectangle $D_i(T_{{\rm req},c})$ (matching the legacy exact-mode treatment, where the window is a hard cap and only the cuts ride the phase ramp). The full-integral rate replaces $V_w$ of Eq.~\eqref{eq:V_w} by the channel sum of the same closed-form pieces: each term of Eq.~\eqref{eq:W_schedule} is piecewise $\{$const $g_c$ $|$ ramp in $T$ $|$ ramp/const in $\tau\}$ with all breakpoint distances given by the existing single-phase inversions ($T=t_{p,\rm eff}[(\tilde D_{\max}/\tilde D)^k-1]$ and Eq.~\eqref{eq:t_lim}), and the same antiderivative family $G(\tilde D)$ with shifted constants. Cost: a Python-level loop over $\le3$ channels inside the existing $q$-chunk loop (temporaries freed between channels), i.e.\ $\approx3\times$ the current rise-on exact-mode cost on discrete-band optical cells only. The numerical medians and the $dR/dq$, $dR/dD$ views inherit the channel sum through the same two entry points ($V_w$-analogue and pointwise $W$).
...
\item Visits are equally spaced at $\Delta t_v$, all inside a top-hat night window, all at equal depth $F_{\rm lim}(t_{\rm exp})$; weather, moon and airmass structure are absorbed into $f_{\rm eff}$.
\item Every visit inside the above-threshold window counts as a detection (as in the legacy criterion); band structure ($g$ vs $r$) is not modelled --- $i$ counts epochs of any band.
\item Single power-law phase for the $t_+$ and $t_{\rm lim}$ inversions ($k=2/|\alpha_\Phi|$, phase-selected), exactly as in the legacy exact mode.
\item The dominant mode keeps the per-channel \emph{endpoint} conventions (worst/zero wait); only the exact mode integrates the interior of the phase ramp. For ZTF-public-like schedules ($\rho=1$, $N_v=2$) the endpoints do not straddle the intra-night pair tightly, so exact mode is the reference there.
\item The sub-day (continuous) band keeps its legacy treatment, including the legacy short rise gap; the physically interesting intra-night modes (e.g.\ ZTF high-cadence) are now expressed on the discrete band ($t_{\rm cad}=1$\,d, $N_v>1$) where the honest overnight rise gap applies.
\item The gap band $t_{\rm night}/i\le t_{\rm cad}<1$\,day remains invalid (deferred: it is representable as the degenerate nightly schedule with $N_v<i$ and multi-night accumulation via $P_i$).
\end{enumerate}

\paragraph{Deferred refinements.} Gap-band modelling; optimiser search over $(N_v,\Delta t_v)$; ramp-averaged dominant mode (bracketed by the two conventions; exact mode has the ramp); closed-form mixture medians; per-band epochs.

\paragraph{Implementation reference.} \texttt{grb\_detect/schedule.py}: \texttt{NightSchedule($N_v$, dt\_v\_s, t\_night\_s)} (frozen), \texttt{spans()} (Eq.~\eqref{eq:spans}: $S_A$, $S_B$, $G$, multiplicities), \texttt{P\_detect()} (Eq.~\eqref{eq:P_i}), \texttt{joint\_weight()} (Eq.~\eqref{eq:W_schedule}), \texttt{feasible()} (Sec.~\ref{sec:sched_feasibility}). \texttt{DetectionRateModel(schedule=...)}; \texttt{make\_rate\_model(f\_eff, N\_v, dt\_v\_s, t\_night\_s, schedule\_on)}; a single model instance replaces \texttt{model\_day}/\texttt{model\_night} (budget Eq.~\eqref{eq:budget_unified} internal, $f_{\rm night}$ factor internal). Bridge parameter keys: \texttt{f\_eff}, \texttt{N\_v}, \texttt{dt\_v\_h} (legacy \texttt{f\_live} accepted as $f_{\rm eff}f_{\rm night}$). Tests: \texttt{tests/test\_schedule\_pi.py} (Eq.~\eqref{eq:P_i} vs brute force), \texttt{tests/test\_schedule\_model.py} ($N_v{=}1$ parity, bracket ordering, budget unification, cut baselines), \texttt{tests/test\_schedule\_parity.py} (pre-change payload snapshots).
