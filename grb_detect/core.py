...
def _make_rate_model_cached(
    ...
    win_from_peak: bool,
    # Night schedule (schedule_on=False → legacy uniform cadence)
    schedule_on: bool,
    N_v: int,
    dt_v_s: float,
    t_night_s: float,
    ...
    schedule = (
        NightSchedule(N_v=int(N_v), dt_v_s=float(dt_v_s), t_night_s=float(t_night_s))
        if schedule_on else None
    )
...
def make_rate_model(
    *,
    ...
    # Night schedule (optical mode; N_v=1 with schedule_on reproduces legacy)
    schedule_on: bool = False,
    N_v: int = 1,
    dt_v_s: float = 0.0,
    t_night_s: float | None = None,
    ...
    t_night_resolved = (
        float(t_night_s) if t_night_s is not None
        else (design.t_night_s if design is not None else _sd.t_night_s)
    )
...
def optical_survey_tcad_seconds(
    ...
    *,
    ...
    Observations are possible only at night, so the only physically meaningful
    cadences are whole numbers of nights:

        t_cad = n * 1 day,  n ∈ ℕ,

    enforced as a validity constraint, not by rounding.  Sub-day sampling is
    not a cadence regime: revisiting a field within the night is what the
    night-schedule parameters (N_v, Δt_v) describe on the n = 1 row (see
    docs/implementation_reference.tex, Sec. "Optical Cadence Constraints";
    the historical continuous/gap bands are subsumed by the schedule).

    ``i_det`` and ``t_night_s`` are kept for API compatibility; the
    per-schedule feasibility masks live in NightSchedule.feasible() and reach
    the rates through DetectionRateModel.t_exp_s.
    ...
    """
    del i_det, t_night_s  # no longer part of the cadence-validity rule
...
def compute_surface(
    ...
    *,
    ...
    Optical mode uses the integer-day cadence grid (the night schedule lives
    in the model itself, so a single model serves every cell); non-optical
    mode is the original continuous grid.
    ...
    # The single model serves every cell, so rate, t_exp and medians always
    # describe the same population (the invariant guarded by
    # tests/test_hover_customdata.py). t_exp uses t_cad_eff to match the rate.
