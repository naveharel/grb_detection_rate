...
- t_cad: cadence, time between consecutive visits to the same field. In
  optical-survey mode this is the inter-night revisit period t_cad = n · 86400 s
  (integer nights) — intra-night revisits, when modeled, are a separate,
  decoupled schedule (N_v visits at spacing dt_v_s; see
  `DetectionRateModel.__init__`'s `N_v`/`dt_v_s` kwargs), not a sub-day t_cad.
...
with N_v = 1 (this module's default) when no intra-night schedule is
attached. The schedule-aware division lives in
`DetectionRateModel.t_exp_s`; the scalar `exposure_time_s` below keeps the
N_v = 1 form for non-schedule callers.
