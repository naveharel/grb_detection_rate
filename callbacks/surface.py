...
def register(app: dash.Dash) -> None:
    ...
    def update_all_visuals(
        ...
        optical_switch, color_switch, tnight_hours, omega_srv_deg2, theme,
    ):
        ...
        if np.isfinite(N_opt) and np.isfinite(t_cad_opt_s):
            try:
                ...
                t_exp_opt_s = float(exposure_time_s(
        ...
        log10_ztf = float(model_day.rate_log10(
            i_det=int(i_det),
        ...
        t_exp_ztf_s = np.nan
        try:
            t_exp_ztf_s = float(exposure_time_s(
